...
| Command | Typical use |
|---|---|
| `systemctl status vsftpd` | Inspect, configure, or validate the ftp area in a repeatable way. |
| `grep -v "^#" /etc/vsftpd/vsftpd.conf` | Inspect, configure, or validate the ftp area in a repeatable way. |
| `setsebool -P ftp_home_dir on` | Inspect, configure, or validate the ftp area in a repeatable way. |
| `firewall-cmd --add-service=ftp --permanent` | Inspect, configure, or validate the ftp area in a repeatable way. |
| `lftp ftp://server` | Inspect, configure, or validate the ftp area in a repeatable way. |
