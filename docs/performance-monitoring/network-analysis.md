...
Network Analysis helps administrators distinguish a real bottleneck from normal activity. Performance work should start with user impact: what is slow, since when, for whom, and compared with what baseline? Tools such as `top`, `sar`, `iostat`, and `ss` are most useful when their output is tied to a timeline.
