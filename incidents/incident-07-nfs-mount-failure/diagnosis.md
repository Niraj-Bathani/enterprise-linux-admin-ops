...
journalctl -u nfs-server -b --no-pager | tail -50
ss -tulpen
getenforce
...
The message matched the reported symptom and pointed to a specific configuration, resource, or dependency. Related checks ruled out broad host failure, DNS-only failure, and unrelated package changes. The likely cause was export entry allowed the wrong subnet after a network migration.
