...
The team needed clearer validation steps for `filesystem`. A process being active was not enough evidence that users could complete their workflow. The runbook should show exact commands, expected output, and common log patterns.

## Follow Up Tasks

- Update the runbook with the log pattern: `kernel: EXT4-fs warning: mounting fs with errors, running e2fsck is recommended`.
